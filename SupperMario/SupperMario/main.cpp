...
#define MAIN_WINDOW_TITLE L"Super Mario Bros 3 - World 1"
...
void ParseSection_MAP_TEXTURES(string line)
{
	vector<string> tokens = split(line);
	...
	LPDIRECT3DTEXTURE9 tex = Textures::GetInstance()->Get(0);
	...
	for (int i = 0; i < row; i++)
	{
		...
		{
			Sprites::GetInstance()->AddSprite(index, j * 16 + x, i * 16 + y, j * 16 + 16 + x, i * 16 + 16 + y, tex);
			index = index + 1;
		}
	}
}
